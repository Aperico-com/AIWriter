...
<?php
  ...
  $results = mysqli_query($con, "SELECT idposts, excerpt, name, prime, freeshipping, fullfilled, itemclass, title, price, brand, availmsg, headline, produrl, prodspec, imgurl, slug, prodtitle, DATE_FORMAT(datein,'%b %D %Y') as dateofpub FROM `reviewposts` WHERE itemclass='". deslugify($tag) ."' ORDER BY datein DESC");
  // echo "SELECT idposts, excerpt, name, prime, freeshipping, fullfilled, itemclass, title, price, brand, availmsg, headline, produrl, prodspec, imgurl, slug, prodtitle, DATE_FORMAT(datein,'%b %D %Y') as dateofpub FROM `posts` WHERE itemclass='". deslugify($tag) ."' ORDER BY datein DESC";
  if (!$results) {
    printf("Error: %s\n", mysqli_error($con));
      exit();
  }
...
<head>
  ...
  <script>
    ...
    ================================================== -->
    <meta charset="utf-8">
    <title><?php echo $siteTitle ?> [<?php echo $tag ?>]</title>
    <meta name="description" content="<?php echo $siteContent ?>">
    <meta name="author" content="<?php echo $rootDomain ?>">
    <meta name="theme-color" content="#000000"/>
    <link rel="canonical" href="https://<?php echo $rootDomain ?>/reviews/category/<?php echo $tag ?>">
...
<body>
  ...
  <div class="container pt-3">
    ...
    <h1 class="mb-1">
        <?php echo ucwords(deslugify($tag)) . " News and Reviews" ?>
    </h1>
